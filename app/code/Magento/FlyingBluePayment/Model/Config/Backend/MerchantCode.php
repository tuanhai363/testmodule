...
class MerchantCode extends Value
{
    /**
    ...
    {
        $merchantCode = trim((string) $this->getValue());
        ...
        try
        {
            $this->helper->getConfig()->saveConfig('payment/flyingblue/merchant_code', $merchantCode, ScopeConfigInterface::SCOPE_TYPE_DEFAULT, 0);
        }
        catch (\Exception $e)
        {
            $this->helper->logError($e, __METHOD__);
            $this->messageManager->addErrorMessage('There was an error while trying to save Merchant Code.');
