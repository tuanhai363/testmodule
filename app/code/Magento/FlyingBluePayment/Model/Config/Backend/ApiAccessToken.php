...
class ApiAccessToken extends Value
{
    /**
    ...
    {
        $apiAccessToken = trim((string) $this->getValue());
        ...
        try
        {
            $this->helper->getConfig()->saveConfig('payment/flyingblue/api_access_token', $apiAccessToken, ScopeConfigInterface::SCOPE_TYPE_DEFAULT, 0);
        }
        catch (\Exception $e)
        {
            $this->helper->logError($e, __METHOD__);
            $this->messageManager->addErrorMessage('There was an error while trying to save API Access Token.');
