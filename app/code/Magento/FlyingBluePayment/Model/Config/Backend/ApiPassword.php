...
class ApiPassword extends Value
{
    /**
    ...
    {
        $apiPassword = trim((string) $this->getValue());
        ...
        try
        {
            $this->helper->getConfig()->saveConfig('payment/flyingblue/api_password', $apiPassword, ScopeConfigInterface::SCOPE_TYPE_DEFAULT, 0);
        }
        catch (\Exception $e)
        {
            $this->helper->logError($e, __METHOD__);
            $this->messageManager->addErrorMessage('There was an error while trying to save API Password.');
